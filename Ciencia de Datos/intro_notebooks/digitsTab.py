...
class digits_representation(Frame):
    ...
    def __init__(self, parent, img_width, img_height, scaler, pca, x_pca, y_train, is_tensor, clas_model):
    ...
    def save(self):
            ...
            img.save('result.png')
            if self.is_tensor:
                x, y = digits_representation.project_tensor(self)
                self.show_proj(x)
                self.show_digit(y)
            else:
                x = digits_representation.project(self)
                x, y = digits_representation.project(self)
                self.show_proj(x)
                self.show_digit(y)
    ...
    def project(self):
        ...
        lab = torch.max(test_output, 1)[1].data.numpy().squeeze()
        return x_digit, lab.item()
    ...
    def project_tensor(self):
        ...
        # obtiene features y categoria           
        im = torch.from_numpy(im).float()        
        test_output, last_layer = self.model(im)
                
        # resize
        img = last_layer.detach().numpy()
        # estandarizar
        new_image = self.scaler.transform(img)
        # proyectar en los componentes calculados
        x_digit = self.pca.transform(new_image)        
