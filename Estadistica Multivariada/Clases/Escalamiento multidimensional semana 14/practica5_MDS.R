...
resul_mds_mc <- smacofSym(disim_nations, ndim=2,type = "ordinal", init = "torgerson", itmax = 1000, eps = 1e-06) 
...
#solucion clasica (coordenadas principales), tambien llamada de Torgerson
...
plot(resul_mds_mc,main = "Grafica del stress por punto (medido en %)", plot.type = "stressplot")
...
#Conclusiones
#El grafico muestra la oposicion que existia entre paises desarrollados y subdesarrollados y paises capitalistas y comunistas
...
#plot(resul_unfold_desayunos,plot.type="resplot")
...
prox_arq <- read.csv("SitiosArqueologicos.csv", header = TRUE, sep = ",", quote = "\"",dec = ".", fill = TRUE, na.strings = "NA")
#nombre de las naciones
nombres<- names(prox_arq)

resul_mds_mc <- smacofSym(prox_arq, ndim=2,type = "ratio", init = "torgerson", itmax = 200, eps = 1e-06) 

config_sitios_mc<-resul_mds_mc$conf

plot(config_sitios_mc[,1],config_sitios_mc[,2],main="configuracion solucion mediante smacof",ylim=range(config_sitios_mc[,1]),
     xlab="Dim 1",ylab="Dim 2",type="n",lwd=2)
text(config_nations_mc[,1],config_nations_mc[,2],
     labels=abbreviate(nombres,minlength=8),cex=0.6,lwd=2)
